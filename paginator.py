...
class PaginatorView(discord.ui.View):
    ...
    def build_ui(self):
        ...
        # Row 1: Pagination & Close
        # Only render the "Previous" arrow if there is more than 1 page
        ...
        # Always keep the Close button so users can delete the search results embed
        close_btn = discord.ui.Button(emoji="❌", style=discord.ButtonStyle.danger, row=1)
        close_btn.callback = self.on_close
        self.add_item(close_btn)

        # Only render the "Next" arrow if there is more than 1 page
        if self.total_pages > 1:
