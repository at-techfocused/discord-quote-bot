...
async def init_db():
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        # Automatic Migration: Add image_url column if it doesn't exist
...
async def add_quote(server_id: str, quote_text: str, added_by_user_id: str, author_user_id: str = None, author_name: str = None, image_url: str = None) -> int:
    quote_id = await get_next_quote_id(server_id)
    timestamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("""
            INSERT INTO quotes (server_id, quote_id, quote_text, author_user_id, author_name, added_by_user_id, timestamp, image_url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (server_id, quote_id, quote_text, author_user_id, author_name, added_by_user_id, timestamp, image_url))
        await db.commit()
    return quote_id
